...
// {
	...
	public class Longbow : MonoBehaviour
	{
		...
		public float nockDistanceTravelled = 0f;
		...
		private void HandAttachedUpdate( Hand hand )
		{
			...
			if ( nocked )
			{
				...
				if ( Vector3.Dot( nockToarrowHand, -nockTransform.forward ) > 0 )
				{
					...
					if ( ( nockDistanceTravelled > ( lastTickDistance + hapticDistanceThreshold ) ) || nockDistanceTravelled < ( lastTickDistance - hapticDistanceThreshold ) )
					{
						...
						tasbi.tactors.VibrateAll(TasbiTactor.Waveform.Square, 175, 0.05f, 0.025f);
						tasbi.otherTasbi.tactors.VibrateAll(TasbiTactor.Waveform.Square, 175, 0.05f, 0.025f);
			...
			// tasbi squeeze
			if (nocked) {
				tasbi.squeeze += nockDistanceTravelled * 2.0f;
				tasbi.otherTasbi.squeeze += nockDistanceTravelled * 2.0f;
			}
		}
		...
		public void ArrowReleased()
		{
			...
			tasbi.tactors.VibrateAll();
			tasbi.otherTasbi.tactors.VibrateAll();
			nockDistanceTravelled = 0;
		...
		public void StartNock( ArrowHand currentArrowHand )
		{
			...
			tasbi.otherTasbi.tactors.VibrateAll(TasbiTactor.Waveform.Sin, 175, 0.1f, 0.025f); 
		}
